...
| `--task-type` | | CHOICE | *(from config)* | Canonicalization type. One of: `numeric`, `mcq`, `llm_judge`, `llm`, `embedding`, `custom`. |
...
| `--task-type` | | CHOICE | **(required)** | Canonicalization type. One of: `numeric`, `mcq`, `llm_judge`, `llm`, `embedding`, `custom`. |
